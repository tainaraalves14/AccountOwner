...
public class OwnerController : ControllerBase
{
    ...
    [HttpPut("{id")]
    private IActionResult UpdateOwner(Guid id, [FromBody] OwnerForUpdateDto owner)
    {
        try
        {
            if (owner is null)
            {
                _logger.LogError("Ojeto Owner enviado está nulo.");
                return BadRequest("Objeto Owner é nulo");
            }

            if (!ModelState.IsValid)
            {
                _logger.LogError("Ojeto owner enviado é inválido.");
                return BadRequest("Objeto de modelo inválido");
            }
